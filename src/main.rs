use std::{net::{IpAddr, UdpSocket}, sync::Arc};

use chainsmoker::{gossip::GossipNode, output::{OutputPlugin, PluginRunner}, shred::ShredReceiver, Keypair, Shred};
...
impl OutputPlugin for ConsolePlugin {
    ...
    async fn handle_shred(&mut self, shred: Shred) -> Result<(), Box<dyn std::error::Error>> {
        println!("Shred: Slot:{} Index:{} Type:{:?}", 
            shred.slot(), shred.index(), shred.shred_type());
        Ok(())
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gossip_node = GossipNode::new(identity_keypair, gossip_socket, &tvu_socket, bind_address)?;
